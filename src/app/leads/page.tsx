...
import { 
  MessageCircle, Phone, Search, Trash2, PartyPopper, 
  CheckCircle2, XCircle, Clock, Calendar, User, 
  Instagram, Globe, Megaphone, MapPin 
} from 'lucide-react'

export default async function LeadsPage() {
  // Busca leads ordenados pelos mais recentes
  const leads = await prisma.lead.findMany({ orderBy: { createdAt: 'desc' } })

  // Função auxiliar para ícone de origem
  const getOrigemIcon = (origem: string) => {
    switch (origem) {
      case 'Instagram': return <Instagram size={12} />;
      case 'Google': return <Globe size={12} />;
      case 'Indicação': return <User size={12} />;
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      {/* --- 1. FORMULÁRIO COMPLETO (COM ORIGEM) --- */}
      <div className="bg-white p-6 rounded-[2rem] border shadow-sm relative overflow-hidden">
        <div className="absolute top-0 right-0 p-4 opacity-5"><User size={100} /></div>
        <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Novo Interessado</h2>
        ...
        <form action={createLead} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end relative z-10">
          <div className="space-y-1">
             ...
             <input name="nome" placeholder="Ex: João Silva" required className="w-full bg-slate-50 p-4 rounded-xl border-none font-bold text-sm outline-blue-500" />
          </div>
          <div className="space-y-1">
             <label className="text-[10px] font-black text-slate-400 uppercase ml-2">WhatsApp</label>
             <input name="telefone" placeholder="(11) 99999-9999" required className="w-full bg-slate-50 p-4 rounded-xl border-none font-bold text-sm outline-blue-500" />
          </div>
          
          {/* CAMPO DE ORIGEM (FORMA DE CONTATO) */}
          <div className="space-y-1">
             <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Origem / Contato</label>
             <div className="relative">
               <select name="origem" className="w-full bg-slate-50 p-4 rounded-xl border-none font-bold text-sm outline-blue-500 appearance-none text-slate-600">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {leads.map((lead) => (
          <div key={lead.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 hover:shadow-xl transition-all group relative overflow-hidden flex flex-col justify-between h-full">
            
            {/* Barra Lateral Colorida do Status */}
            <div className={`absolute left-0 top-0 bottom-0 w-3 ${
              lead.status === 'FECHADO' ? 'bg-emerald-500' : 
              lead.status === 'PERDIDO' ? 'bg-red-400' : 'bg-blue-500'
            }`}></div>
            ...
            <div className="pl-4">
              <div className="flex justify-between items-start mb-2">
                ...
                <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider border ${
                  lead.status === 'FECHADO' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 
                  lead.status === 'PERDIDO' ? 'bg-red-50 text-red-500 border-red-100' : 'bg-blue-50 text-blue-600 border-blue-100'
                }`}>
                  {lead.status}
                </span>
              </div>
            ...
            <div className="pl-4 mt-auto pt-4 border-t border-slate-100 flex justify-between items-center gap-2">
              ...
              <div className="flex gap-1">
                {lead.status !== 'FECHADO' && (
                  <form action={updateLeadStatus}>
                    ...
                    <button title="Venda Fechada" className="p-2 rounded-xl bg-slate-50 text-slate-300 hover:text-white hover:bg-emerald-500 transition shadow-sm">
                ...
                {lead.status !== 'PERDIDO' && (
                  <form action={updateLeadStatus}>
                    ...
                    <button title="Perdido" className="p-2 rounded-xl bg-slate-50 text-slate-300 hover:text-white hover:bg-red-400 transition shadow-sm">
                        <XCircle size={18}/>
                    </button>
                  </form>
                )}
              ...
              <div className="flex gap-2">
                <Link 
                  href={`/festas/nova?nomeCliente=${encodeURIComponent(lead.nome)}&telefoneCliente=${encodeURIComponent(lead.telefone)}`}
                ...
                <form action={deleteLead}>
                  <input type="hidden" name="id" value={lead.id} />
                  <button className="p-2 rounded-xl bg-slate-50 text-slate-300 hover:text-white hover:bg-red-500 transition shadow-sm" title="Excluir">
                    <Trash2 size={18} />
                  </button>
                </form>
              </div>
