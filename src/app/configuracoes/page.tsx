...
import { ShieldCheck, UserPlus, Save, Users, Lock, Shield } from 'lucide-react'
  ...
  return (
    <div className="space-y-8 pb-20 max-w-7xl mx-auto">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-slate-400 font-medium">Gerencie credenciais e níveis de permissão da sua equipe.</p>
      </header>
      ...
      <section className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 shadow-2xl">
        ...
        <form action={createUsuario} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
          <div className="space-y-2">
            ...
            <input name="nome" placeholder="Ex: Ana Souza" required className="w-full bg-slate-950 p-4 rounded-2xl border border-slate-800 font-bold text-sm text-white focus:ring-2 focus:ring-emerald-500 outline-none transition-all" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase ml-2 tracking-widest">Email (Login)</label>
            <input name="email" type="email" placeholder="ana@buffet.com" required className="w-full bg-slate-950 p-4 rounded-2xl border border-slate-800 font-bold text-sm text-white focus:ring-2 focus:ring-emerald-500 outline-none transition-all" />
          </div>
          <div className="space-y-2">
             <label className="text-[10px] font-black text-slate-500 uppercase ml-2 tracking-widest">Senha</label>
             <input name="senha" type="password" placeholder="******" required className="w-full bg-slate-950 p-4 rounded-2xl border border-slate-800 font-bold text-sm text-white focus:ring-2 focus:ring-emerald-500 outline-none transition-all" />
          </div>
          <div className="space-y-2">
             <label className="text-[10px] font-black text-slate-500 uppercase ml-2 tracking-widest">Cargo / Função</label>
             <select name="cargo" className="w-full bg-slate-950 p-4 rounded-2xl border border-slate-800 font-bold text-sm text-slate-300 focus:ring-2 focus:ring-emerald-500 outline-none appearance-none cursor-pointer">
          ...
          <button type="submit" className="bg-emerald-600 text-white font-black px-6 py-4 rounded-2xl uppercase text-xs tracking-widest hover:bg-emerald-500 transition shadow-lg shadow-emerald-900/20 flex items-center justify-center gap-2 group">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4">
          {usuarios.map(u => (
            <div key={u.id} className="bg-slate-900 p-6 rounded-[2.5rem] border border-slate-800 hover:border-emerald-500/30 transition-all duration-300">
              <form action={updatePermissoes}>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                  ...
                  <div className="flex items-center gap-4 min-w-[250px]">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] px-2 py-0.5 bg-slate-800 text-slate-400 rounded-md font-bold">{u.cargo}</span>
                  ...
                  {/* Ação */}
                  {u.cargo !== 'DONO' && (
                    <button type="submit" className="bg-white text-slate-950 p-4 rounded-2xl hover:bg-emerald-400 hover:scale-105 transition-all shadow-xl shadow-white/5" title="Aplicar Permissões">
                      <Save size={20} />
                    </button>
                  )}
                </div>
...
function PermissionToggle({ label, name, checked, color }: any) {
  // Mapeamento para evitar erro de build no Tailwind
  const colorMap: any = {
  ...
  return (
    <label className="flex items-center gap-2 cursor-pointer bg-slate-950 px-4 py-2 rounded-xl border border-slate-800 hover:border-slate-600 transition-all select-none group">
      ...
      />
      <span className={`text-[10px] font-black uppercase tracking-widest ${colorMap[color] || "text-slate-400"}`}>{label}</span>
    </label>
  )
}