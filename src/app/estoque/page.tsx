      ...
      <header className="flex flex-col md:flex-row justify-between items-end gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-800 uppercase italic flex items-center gap-3">
            <PackageSearch className="text-yellow-500" size={32} /> Controle de
            Estoque
          </h1>
          <p className="text-slate-500 font-medium">
            Gerencie entradas e saídas de materiais.
          </p>
        </div>
          ...
          <div className="bg-red-50 px-4 py-2 rounded-xl border border-red-100 flex items-center gap-3 animate-pulse">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-red-400">
                {estoqueBaixo.length} itens precisam de reposição.
              </p>
            </div>
          </div>
        )}
        ...
        >
          <div className="w-full space-y-1">
            ...
            <input
              ...
              required
              className="w-full bg-slate-50 p-3 rounded-xl border-none font-bold text-sm"
            />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {itens.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`bg-white p-6 rounded-[2rem] border transition hover:shadow-xl relative overflow-hidden ${
            ...
            >
              ...
              <div className="mb-4">
                ...
                <h3 className="font-black text-xl text-slate-800 mt-2 truncate">
                  {item.nome}
                </h3>
              </div>
              ...
              <div className="bg-slate-50 rounded-2xl p-4 text-center mb-4">
                ...
                >
                  {item.quantidade}{" "}
                  <span className="text-sm text-slate-400">un</span>
                </div>
              </div>

              <div className="border-t border-slate-100 pt-4">
                <div className="text-[9px] font-black text-slate-400 uppercase mb-2 ml-1">
                  Movimentação Rápida
                </div>
