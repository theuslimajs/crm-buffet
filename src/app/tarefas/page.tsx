...
export default async function TarefasPage() {
  // Busca tarefas ordenadas por data (as mais urgentes primeiro)
  const tarefas = await prisma.tarefa.findMany({
    orderBy: [
      { status: 'asc' }, // Pendentes primeiro
      { dataLimite: 'asc' } // Mais urgentes primeiro
    ]
  })
      ...
      <div className="bg-white p-6 rounded-[2rem] border shadow-sm">
        ...
        <form action={createTarefa} className="flex flex-col md:flex-row gap-3 items-end">
          <div className="w-full space-y-1">
             <label className="text-[10px] font-black text-slate-400 uppercase ml-2">O que precisa ser feito?</label>
             <input name="descricao" placeholder="Ex: Comprar bebidas para festa do João" required className="w-full bg-slate-50 p-3 rounded-xl border-none font-bold text-sm text-slate-700" />
          </div>
          
          <div className="w-full md:w-48 space-y-1">
             <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Quem?</label>
             <select name="equipe" className="w-full bg-slate-50 p-3 rounded-xl border-none font-bold text-sm text-slate-600">
               <option>Geral</option>
          ...
          <div className="w-full md:w-40 space-y-1">
             ...
             <input name="dataLimite" type="date" required className="w-full bg-slate-50 p-3 rounded-xl border-none font-bold text-sm text-slate-500" />
          </div>

          <button className="bg-slate-900 text-white font-black px-6 py-3 rounded-xl uppercase text-xs tracking-widest hover:bg-slate-800 transition shadow-lg flex gap-2 items-center justify-center w-full md:w-auto">
            <Plus size={16} /> Criar
          </button>
        </form>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          {pendentes.map(tarefa => (
            <div key={tarefa.id} className="bg-white p-4 rounded-2xl border border-slate-100 hover:shadow-md transition flex items-center justify-between group">
               <div className="flex items-start gap-3">
                 {/* Botão Check */}
                 <form action={toggleTarefaStatus}>
                   <input type="hidden" name="id" value={tarefa.id} />
                   <button className="mt-1 text-slate-300 hover:text-emerald-500 transition">
                     <Circle size={20} />
                   </button>
                 </form>
                 ...
                 <div>
                   ...
                   <div className="flex gap-3 mt-1">
                     ...
                     <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                       <Calendar size={10} /> {new Date(tarefa.dataLimite).toLocaleDateString('pt-BR')}
                     </span>
                   </div>
                 </div>
               </div>
               ...
               <form action={deleteTarefa}>
                 <input type="hidden" name="id" value={tarefa.id} />
                 <button className="text-slate-200 hover:text-red-400 transition p-2">
                   <Trash2 size={16} />
                 </button>
               </form>
            </div>
        ...
        <div className="space-y-4 opacity-70 hover:opacity-100 transition duration-500">
          ...
          {concluidas.map(tarefa => (
            <div key={tarefa.id} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center justify-between">
