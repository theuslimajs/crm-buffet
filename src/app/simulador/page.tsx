...
export default function SimuladorPage() {
  ...
  // CUSTOS
  const [custoCarne, setCustoCarne] = useState<number>(0);
  const [custoBebida, setCustoBebida] = useState<number>(0);
  const [custoEquipe, setCustoEquipe] = useState<number>(0);
  const [custoDecoracao, setCustoDecoracao] = useState<number>(0);
    ...
    <main className="min-h-screen bg-slate-950 p-8 text-slate-100">
      ...
      <form action={salvarSimulacao} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-slate-500">Carnes/Comida</label>
                <input
                  type="number"
                  step="0.01"
                  min={0}
                  value={custoCarne}
                  onChange={(e) => setCustoCarne(parseFloat(e.target.value) || 0)}
                  className="w-full bg-slate-800 p-3 rounded-xl font-bold"
                />
              </div>

              <div>
                <label className="text-xs text-slate-500">Bebidas</label>
                <input
                  type="number"
                  step="0.01"
                  min={0}
                  value={custoBebida}
                  onChange={(e) => setCustoBebida(parseFloat(e.target.value) || 0)}
                  className="w-full bg-slate-800 p-3 rounded-xl font-bold"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  min={0}
                  value={custoEquipe}
                  onChange={(e) => setCustoEquipe(parseFloat(e.target.value) || 0)}
                  className="w-full bg-slate-800 p-3 rounded-xl font-bold"
                />
              </div>

              <div>
                <label className="text-xs text-slate-500">Decoração</label>
                <input
                  type="number"
                  step="0.01"
                  min={0}
                  value={custoDecoracao}
                  onChange={(e) => setCustoDecoracao(parseFloat(e.target.value) || 0)}
                  className="w-full bg-slate-800 p-3 rounded-xl font-bold"
                />
              </div>

              <div className="col-span-2">
                <label className="text-xs text-slate-500">Outros custos</label>
                <input
                  type="number"
                  step="0.01"
                  min={0}
            ...
            {/* hidden fields pro servidor */}
            <input type="hidden" name="custo" value={String(custoTotal)} />
            <input type="hidden" name="lucroEstimado" value={String(lucro)} />
            <input type="hidden" name="margem" value={String(margem)} />
            <input type="hidden" name="detalhes" value={JSON.stringify(detalhes)} />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div
            className={`flex-1 p-8 rounded-3xl border shadow-2xl flex flex-col items-center justify-center text-center ${
              lucro >= 0 ? "bg-emerald-500/10 border-emerald-500/50" : "bg-red-500/10 border-red-500/50"
            }`}
          >
            ...
            <div className="w-full bg-slate-800 rounded-full h-4 overflow-hidden mb-2">
              <div
                className={`h-full ${margem > 30 ? "bg-emerald-500" : "bg-yellow-500"}`}
          ...
          <button
            type="submit"
            className="w-full bg-white text-slate-950 font-black p-4 rounded-xl flex items-center justify-center gap-2 hover:bg-slate-200 transition"
          >
